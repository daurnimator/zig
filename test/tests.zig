...
pub const CompareOutputContext = struct {
    ...
    const RunCompareOutputStep = struct {
        ...
        fn make(step: *build.Step) !void {
            ...
            child.stdout.?.readAllBuffer(&stdout, max_stdout_size) catch unreachable;
            child.stderr.?.readAllBuffer(&stderr, max_stdout_size) catch unreachable;
...
pub const CompileErrorContext = struct {
    ...
    const CompileCmpOutputStep = struct {
        ...
        fn make(step: *build.Step) !void {
            ...
            child.stdout.?.readAllBuffer(&stdout_buf, max_stdout_size) catch unreachable;
            child.stderr.?.readAllBuffer(&stderr_buf, max_stdout_size) catch unreachable;
